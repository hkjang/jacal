...
export default function HabitsAdmin() {
  ...
  const { data: habitsData, isLoading } = useQuery({
    queryKey: ['admin', 'habits', page, limit, search],
    queryFn: () => adminAPI.getHabits({ page, limit, search }),
  });
  ...
  return (
    <div className="admin-section">
      <div className="section-header">
        <div>
          <h2>{t('admin.habits', '습관')}</h2>
          <p className="section-description">All user habits across the system</p>
        </div>
          ...
          <tbody>
            {habitsData?.data?.map((habit: any) => (
              <tr key={habit.id}>
                ...
                <td><span className="badge">{habit.frequency}</span></td>
                <td>{habit.targetDays} days</td>
